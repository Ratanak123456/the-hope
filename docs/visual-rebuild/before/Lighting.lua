...
local function applyMood(props: { [string]: any }, atmosphere: { [string]: any }?, cloud: { [string]: any }?, grade: { [string]: any }?)
	setProps(Lighting, props)
 withEffect(Lighting, "HopeBloom", function(instance) setProps(instance, {Intensity=0.18, Size=18, Threshold=1.6}) end)
...
function NPLighting.exterior()
	applyMood({
		...
		Ambient = Color3.fromRGB(58, 64, 74),
		OutdoorAmbient = Color3.fromRGB(120, 130, 142),
		ColorShift_Top = Color3.fromRGB(30, 34, 40),
		ColorShift_Bottom = Color3.fromRGB(10, 14, 22),
		FogColor = Color3.fromRGB(150, 160, 172),
		FogStart = 40,
		FogEnd = 260,
	}, {
		Haze = 3.2,
...
-- Warm practical lights inside the command tent, contrasting the cold
-- exterior - "warm inside vs. cold outside."
function NPLighting.commandTent()
	applyMood({
		ClockTime = 9.2,
		Brightness = 1.1,
		ExposureCompensation = 0.05,
		Ambient = Color3.fromRGB(70, 58, 44),
		OutdoorAmbient = Color3.fromRGB(90, 96, 108),
		ColorShift_Top = Color3.fromRGB(40, 30, 14),
		ColorShift_Bottom = Color3.fromRGB(10, 10, 16),
		FogColor = Color3.fromRGB(40, 34, 28),
		FogStart = 10,
		FogEnd = 90,
	}, {
		Haze = 1.2,
		...
		Decay = Color3.fromRGB(90, 80, 70),
	}, nil, {
		Brightness = 0,
		Contrast = 0.08,
		Saturation = -0.1,
		TintColor = Color3.fromRGB(255, 236, 210),
	})
end

-- Ancient interior: cool, deep, teal-lit, room tone dark. Used for the
...
-- promising the core beneath. `awaken` (0..1) warms it as the core ignites.
function NPLighting.chamber(awaken: number?)
	local a = awaken or 0
	applyMood({
		ClockTime = 0,
		Brightness = 0.4 + 0.45 * a,
		ExposureCompensation = -0.12 + 0.15 * a,
...
-- Violet-tinted alien prison cavern.
function NPLighting.prison()
	applyMood({
		ClockTime = 0,
		Brightness = 0.32,
		ExposureCompensation = -0.15,
		Ambient = Color3.fromRGB(52, 52, 80),
		OutdoorAmbient = Color3.fromRGB(28, 18, 34),
